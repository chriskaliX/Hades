...
/* l4_acl_rule
 * 
 * The L4 ACL rule would deny or record the packet by it's iphdr.
 * Only UDP and TCP are supported now(port information), other protocols will be
 * supported in the feature.
 */
static __always_inline int l4_acl_rule(net_packet_t pkt, struct __sk_buff *skb) {
    struct policy_key key = {0};
    key.prefixlen = 128;

    if (pkt.ctx.ingress == 0) {
        key.addr = pkt.ctx.dst_addr;
    } else {
        key.addr = pkt.ctx.src_addr;
    }

    struct policy_value *value = bpf_map_lookup_elem(&policy_map, &key);
    if (value && value->ingress == pkt.ctx.ingress) {
        size_t pkt_size = sizeof(pkt);
        // protocol match, port is ignored
        if (value->protocol != PROTOCOL_ALL && value->protocol != pkt.ctx.protocol) {
            return TC_ACT_UNSPEC;
        }
        // for now, only support udp & tcp
        if (pkt.ctx.protocol == IPPROTO_TCP || pkt.ctx.protocol == IPPROTO_UDP) {
            if (port_check(value, pkt.ctx.dst_port) == false) {
                return TC_ACT_UNSPEC;
            }
        }
        if (value->action == ACTION_DENY) {
            pkt.ctx.action = ACTION_DENY;
