...
int tp_execve(struct syscall_enter_args *ctx)
{
...
int tp_execveat(struct syscall_enter_args *ctx)
{
    __u64 id = bpf_get_current_pid_tgid();
    struct syscall_buffer *buf = reset_syscall_buffer_cache(id);
    if (!buf)
...
int tp_prctl(struct syscall_enter_args *ctx)
{
    event_data_t data = {};
    if (!init_event_data(&data, ctx))
        return 0;
...
int tp_ptrace(struct syscall_enter_args *ctx)
{
    event_data_t data = {};
    if (!init_event_data(&data, ctx))
        return 0;
...
int tp_memfd_create(struct syscall_enter_args *ctx)
{
    event_data_t data = {};
    if (!init_event_data(&data, ctx))
        return 0;
