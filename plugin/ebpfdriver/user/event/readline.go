...
func (r *Readline) DecodeEvent(decoder *decoder.EbpfDecoder) (err error) {
	if r.Exe, err = decoder.DecodeString(); err != nil {
		return
	}
	if r.Line, err = decoder.DecodeString(); err != nil {
		return
	}
	if r.TTYName, err = decoder.DecodeString(); err != nil {
		return
	}
	if r.Stdin, err = decoder.DecodeString(); err != nil {
		return
	}
	if r.Stout, err = decoder.DecodeString(); err != nil {
		return
	}
	if r.Family, r.Dport, r.Dip, err = decoder.DecodeRemoteAddr(); err != nil {
		return
	}
	if r.PidTree, err = decoder.DecodePidTree(&r.PrivEscalation); err != nil {
		return
	}
	if r.Cwd, err = decoder.DecodeString(); err != nil {
		return
	}
	return
}
...
func (r Readline) GetProbes() []*manager.Probe {
...
// func init() {
// 	decoder.RegistEvent(DefaultReadline)
// }
